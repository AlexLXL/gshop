...
<script>
import RatinsFilter from "./RatingsFilter.vue";
  ...
  computed: {
    ...
    filterRatings() {
      ...
      return ratings.filter(item => {
        const { rateType } = item;
        if (selectType === 2) {
          return !onlyContent || item.text.length > 0; //onlyContent为false的时候，取反就是true，表示全部满足条件，全部返回
        } else {
          return (
            selectType == rateType && (!onlyContent || item.text.length > 0)
          );
        }
      });
    }
